...
assessing_origin_institution <- 
  only_european_authors_leidenonly %>% 
  filter(lr_region == "eu" & order_of_publishing == 1) %>% 
  select(cluster_id, lr_univ_name) %>% 
  left_join(publication_list_all, by = c("cluster_id", "lr_univ_name")) %>% 
  group_by(cluster_id, lr_univ_name) %>% 
  summarise(number_of_publications_with_this_affilation = n()) %>% 
  group_by(cluster_id) %>% 
  mutate(number_of_starting_leiden_affilations = n_distinct(lr_univ_name)) %>% 
  arrange(cluster_id, desc(number_of_publications_with_this_affilation)) %>% 
  mutate(origin_institution = first(lr_univ_name)) %>%  #this makes a variable of the origin institution. If there were multiple leiden institutions at time 0, then this takes the instituion where the researcher had the most publications. if it is a tie, then it is selected alphabetically %>% 
  select(cluster_id, origin_institution) %>% 
  distinct(cluster_id, .keep_all = T) %>% 
  left_join(only_european_authors_leidenonly, by ="cluster_id") %>% 
  filter(origin_institution == lr_univ_name) %>% 
...
movers_24_months_min <- movers_to_USA_dataset %>% #taking the dataset of researchers who moved to the USA as their first ever abroad affilation
  ...
  mutate(years_between_starting_and_moving = pub_year-first_year) %>% #assess the months betweeen starting academia (first publication) and the month that they moved to the USA
  filter(career_length_months_at_this_pub >= 24) %>% #then we only keep participants who moved at least 24 months into their career
  select(cluster_id, origin_country,move_to_USA_publication_order = order_of_publishing, move_to_USA_year = pub_year, years_between_starting_and_moving, months_between_starting_and_moving = career_length_months_at_this_pub) %>% #i'm selecting and renaming certain columns here to neaten things up
...
length(unique(movers_24_months_min$cluster_id)) #4835
...
movers_with_same_instution <- movers_24_months_min %>% # so for all of our eligible movers...
  filter(order_of_publishing == move_to_USA_publication_order-1, #... we check that at the publication before the publication showing they had moved ot the USA...
         lr_univ_name == origin_institution) %>% ###they were still affilated with their origin institution. The helps us know that they haven't moved loads before moving to the USA
...
length(unique(movers_with_same_instution$cluster_id)) #4089
...
length(unique(only_usa_destination$cluster_id)) #3964
...
#### because otherwise we don't have enough time to measure citation performance. We go with before or in 2015 (then at the least we can see performance in 2015,2016,2017,2018,2019)

movers2015_or_earlier <- only_usa_destination_leiden_ranked_single %>% 
  filter(move_to_USA_year <= 2015)

length(unique(movers2015_or_earlier$cluster_id)) #2595

### then we only include people that were at the destination for at least 24 months

movers_at_destination_24monthsmin <- movers2015_or_earlier %>%   #so of our eligible movers we...
  distinct(cluster_id, .keep_all = T) %>% #.... take those cluster_ids and...
  select(cluster_id, origin_country, move_to_USA_publication_order,move_to_USA_year,USA_institution = lr_univ_name, USA_lr_univ_id = lr_univ_id,years_between_starting_and_moving, months_between_starting_and_moving) %>%
  ...
  filter(lr_univ_id == USA_lr_univ_id) %>% # taking only rows where the mover if at their destination
  ...
  mutate(final_publication_at_destination_year = pub_year, #a new variable: what year was the last published article
         final_publication_at_destination_month = months_numeric_null_is_min_of_year, #new variable: what month of the year was the last published article
...
movers_dataset_final <- movers_at_destination_24monthsmin
  
#how many eligible movers to USA do have?
length(unique(movers_dataset_final$cluster_id)) #1559
...
researcher_performance <- final_complete_dataset %>% 
  ...
  summarise(discipline = first(discipline),
            month = first(career_length_months_at_this_pub),
            p_full = sum(p_full),
            p_frac = sum(p_frac),
            cs_frac = sum(cs_frac),
...
individual_researcher_performance_months <- researcher_performance %>% 
  ...
  select(cluster_id:discipline, cum_p_full:cum_njs_frac, -career_length_months_at_this_pub)
